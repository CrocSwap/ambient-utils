...
export const fetchTokenBalances = async (
  ...
  const covalentChainString = COVALENT_CHAIN_IDS[chain as keyof typeof COVALENT_CHAIN_IDS] || 'eth-mainnet';

  const covalentBalancesResponse = await client.BalanceService.getTokenBalancesForWalletAddress(
  ...
  const getTokenInfoFromCovalentBalance = (tokenBalance: any): TokenIF => {
    const tokenBalanceBigNumber = BigNumber.from(tokenBalance.balance.toString());
    ...
    return {
      ...
      address:
        tokenBalance.contract_address === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee'
          ? ZERO_ADDRESS

...
import { CandlesByPoolAndDurationIF, CandleDataIF, CandleDataServerIF } from '../types/candleData';
...
export async function fetchCandleSeriesHybrid(
  ...
  try {
    const stitchTime = Math.floor(Math.min(endTime, ...candles.candles.map((c) => c.time)));
  ...
  return payload
    ...
    .map((p) => {
      ...
      return {
        ...
        volumeUSD: (p.volumeBase * baseUsdMult + p.volumeQuote * quoteUsdMult) / 2.0,
        ...
        invPriceOpenExclMEVDecimalCorrected: 1 / (p.priceOpen * priceDecMult),
        invPriceCloseExclMEVDecimalCorrected: 1 / (p.priceClose * priceDecMult),
      };
    });
}
...
async function fetchCandleSeriesUniswap(
  ...
  const { baseToken: mainnetBase, quoteToken: mainnetQuote } = translateUniswapTokens(
